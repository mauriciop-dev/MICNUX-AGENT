...
async function sendEmail(to, subject, body) {
  try {
    if (!process.env.GMAIL_APP_PASSWORD) {
      return "ERROR_AUTH: Falta GMAIL_APP_PASSWORD en Vercel.";
    }
    const mailOptions = { from: `Micnux Agent <${USER_EMAIL}>`, to, subject, html: body, replyTo: USER_EMAIL };
    const info = await transporter.sendMail(mailOptions);
    return `CORREO_ENVIADO: ${info.messageId}. El destinatario es ${to}.`;
...
async function createCalendarEvent(summary, startTime, endTime, description = "") {
  try {
    ...
    return `EVENTO_AGENDADO: ${res.data.htmlLink}. El evento está en tu calendario personal.`;
  } catch (error) { return `ERROR_CALENDAR: ${error.message}`; }
}

async function listEmails(maxResults = 2) {
  const gmailObj = google.gmail({ version: 'v1', auth });
  try {
    const res = await gmailObj.users.messages.list({ userId: 'me', maxResults });
    return res.data.messages ? "Tienes correos pendientes." : "Bandeja vacía.";
  } catch (e) { return `ERROR_GMAIL_READ: ${e.message}`; }
}

const appendSheetRow = (spreadsheetId, range, values) => sheets.spreadsheets.values.append({ spreadsheetId, range, valueInputOption: 'USER_ENTERED', requestBody: { values: [values] } });
const createDocument = (title) => docs.documents.create({ requestBody: { title } });

// LIBERACIÓN DE EXPORTS 🚀✨🍾
module.exports = { 
  drive, sheets, searchFiles, readSheet, 
  sendEmail, createCalendarEvent, listEmails, 
  appendSheetRow, createDocument 
};
