...
// ID del Calendario del Usuario (Mauricio)
...
async function listEmails(maxResults = 2) {
  try {
    ...
    return messages;
  } catch (error) { return `ERROR_GMAIL_READ: ${error.message}. Verifica que la 'Gmail API' esté habilitada.`; }
}

/**
 * Lista los próximos eventos del calendario compartido.
 */
async function listCalendarEvents(maxResults = 5) {
  try {
    const res = await calendar.events.list({
      calendarId: USER_CALENDAR_ID, // <-- USAR EL CALENDARIO COMPARTIDO
  ...
  } catch (error) { return `ERROR_CALENDAR_READ: ${error.message}`; }
}
...
async function searchFiles(query = "") {
  try {
    // Limpiamos la query para evitar fallos por espacios o caracteres raros
    const cleanQuery = query.replace(/_/g, " ").trim();
    const res = await drive.files.list({
      q: `name contains '${query}' or name contains '${cleanQuery}' and trashed = false`,
...
// FUNCIONES EXISTENTES (Sheets, Gmail, Docs)
